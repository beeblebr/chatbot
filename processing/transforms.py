...
def BucketizeIntoSimilarityIntervals(similarity_map):
    intervals = reversed(np.arange(0.65, 1.0,
                                   0.05))  # Divide (0.65, 1.0) into intervals of 0.05 (in reverse order)
    buckets = []
    for lower_bound in intervals:
        bucket = [ki for ki in similarity_map if
                  ki['cosine_similarity'] > lower_bound]
