...
client = MongoClient('mongo', username=conf.MONGO_USERNAME,
...
def update_knowledge_item(eight_id, original_text, updated_text):
    ...
    db.knowledge.update({'eight_id': eight_id, 'text': original_text}, {
        '$set': {'text': updated_text, 'transformed_text': transformed_text}})
...
def clear_questions_history_for_user(eight_id):
    db.users.update({'eight_id': eight_id}, {'$unset': {'questions': 1}},
...
def get_questions_since(start_time):
    return db.history.find({'timestamp': {'$gt': start_time}}).sort(
        'timestamp')
