...
def weighted_decay_sum(counts, verbose=False):
    ...
    for i in reversed(range(len(counts))):
        ...
        exponent = (len(counts) - i - 1) / DECAY_HALF_LIFE  # -1 to correct for zero-indexing
...
def identify_trending_topics():
    ...
    all_topics = set(flatten([get_all_topics(x['transformed_text'], transformed=True) for x in history]))

    topic_wise_counts = {}
    for topic in all_topics:
        num_mentions = []
        for questions in buckets:
            num_mentions.append(len([x for x in questions if topic in x['transformed_text']]))
